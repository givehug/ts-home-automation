...
class Camera {
	...
	*/
    readImages() {
        return new Promise((res, rej) => {
            fs.readdir(this.imagesDir, (err, fileNames) => {
                if (err) {
                ...
                if (fileNames.length > this.imagesLimit) {
                    fileNames.length = this.imagesLimit;
                }

                fileNames.forEach(file => {
                    fs.readFile(this.imagesDir + '/' + file, (err, content) => {
                        if (err) {
	...
	*/
    deletePictures() {
        return new Promise((res, rej) => {
            fs.emptyDir(this.imagesDir, err => {
				if (err) {
